...
def _extract_fwd_bwd_outputs(joint_module: fx.GraphModule, graph: torch.fx.graph.Graph):
    num_fwd_outputs = joint_module._out_spec.children_specs[0].num_leaves
    outputs = pytree.tree_flatten([node.args for node in graph.nodes if node.op == 'output'])[0]
    fwd_outputs = outputs[:num_fwd_outputs]
    bwd_outputs = outputs[num_fwd_outputs:]
    return fwd_outputs, bwd_outputs


def _extract_fwd_bwd_modules(joint_module: fx.GraphModule, graph: torch.fx.graph.Graph, saved_values):
    fwd_outputs, bwd_outputs = _extract_fwd_bwd_outputs(joint_module, graph)
    primal_inputs = list(filter(_is_primal, graph.nodes))
    tangent_inputs = list(filter(_is_tangent, graph.nodes))
    # Construct the forward module
    fwd_graph = _extract_graph_with_inputs_outputs(graph, primal_inputs, fwd_outputs + saved_values)
    bwd_graph = _extract_graph_with_inputs_outputs(graph, saved_values + tangent_inputs, bwd_outputs)
...
def default_partition(
    ...
    fwd_outputs, bwd_outputs = _extract_fwd_bwd_outputs(joint_module, joint_module.graph)
    ...
    return _extract_fwd_bwd_modules(joint_module, joint_module.graph, saved_values)
...
def min_cut_rematerialization_partition(
    ...
    optimized_graph = fx_graph_cse(fx_g)
    # joint_module.graph = optimized_graph
    ...
    def classify_nodes(optimized_graph):
        ...
        primal_inputs = list(filter(_is_primal, optimized_graph.nodes))
        fwd_outputs, _ = _extract_fwd_bwd_outputs(joint_module, optimized_graph) 
        forward_only_graph = _extract_graph_with_inputs_outputs(optimized_graph, primal_inputs, fwd_outputs)
        required_fw_nodes = {name_to_node[node.name] for node in forward_only_graph.nodes
                             if node.op != 'output'}
        unclaimed_nodes = {node for node in optimized_graph.nodes
                           if node not in required_fw_nodes and node not in required_bw_nodes}
        return required_fw_nodes, required_bw_nodes, unclaimed_nodes

    required_fw_nodes, required_bw_nodes, unclaimed_nodes = classify_nodes(optimized_graph)
    for node in reversed(optimized_graph.nodes):

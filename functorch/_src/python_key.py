...
from functorch._C import hasPythonKey, addPythonKey, removePythonKey, _pythonkey_increment_nesting, _pythonkey_decrement_nesting, _set_proxy_function
...
def create_proxy_function(tracer):
    def factory_torch_function(func, types, args=(), kwargs={}):
        namespace, func_name = func.split("::")
        func = getattr(getattr(torch.ops, namespace), func_name)
        outs = kwargs['val']
        rets = []
        out_proxy = tracer.create_proxy('call_function', func, args, {})
        return [PythonTensor(outs[0], out_proxy)]
    return factory_torch_function
...
def placeholder(*args):
    raise RuntimeError("PythonKey mode key somehow triggered - should not happen...")

_set_proxy_function(placeholder)
def pythonkey_trace(root : Union[torch.nn.Module, Callable], concrete_args: Optional[Dict[str, Any]] = None) -> GraphModule:
    tracer = PythonKeyTracer()
    try:
        _pythonkey_increment_nesting()
        _set_proxy_function(create_proxy_function(tracer))

...
from functorch._src.remat_utils_weighted import get_users, get_fused_node_pairs, get_num_changes, rematerialize, copy_all_nodes
...
class GetNumChangesPointwiseTestCase(TestCase):
    ...
    def test_multiple_fused_groups(self):
        ...
        add_num_placeholder, remove_num_placeholder, delta_write \
            = get_num_changes(node_pair, node_users_map, fused_graph)
        self.assertEqual(add_num_placeholder, 4*8, f"add_num_placeholder is {add_num_placeholder}")
        self.assertEqual(remove_num_placeholder, 1*8, f"remove_num_placeholder is {remove_num_placeholder}")
        self.assertEqual(delta_write, 0*8, f"delta_write is {delta_write}")
